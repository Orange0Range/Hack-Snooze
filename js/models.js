...
class StoryList {
  ...
  async removeStory(user, story){
    ...
    const sID = story.storyId
    const res = await axios({
      method: 'POST',
      url: `${BASE_URL}/stories/${sID}`,
      data:
      {
        token        
      }
  ...
  async addStory(user, newStory) {
    const token = user.loginToken;
    const subTitle = newStory.title;
    const subAuthor = newStory.author;
    const subURL = newStory.url;
    let returnStory = await axios({
      method: 'POST',
      url: `${BASE_URL}/stories`,
      data:
      {
    ...
    //console.log(returnStory.data.story)
    const{storyId, title, author, url, username, createdAt} = returnStory.data.story;
    //console.log(title)
